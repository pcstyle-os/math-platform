    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-16 sm:py-20 w-full">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-12 sm:mb-16">
          <div>
            <h1 className="text-5xl font-extrabold mb-3 tracking-tight text-[var(--foreground)]">
              {getLabel("projects")}
            </h1>
            <p className="text-[var(--text-muted)] font-medium text-lg max-w-xl">
              {isCyber
                ? "// Zarządzaj swoimi planami nauki"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {stats && (
            <>
              <div
                className={`p-6 flex items-center gap-6 ${isCyber ? "border border-[var(--primary)] bg-[var(--primary)]/5" : "bg-gradient-to-br from-orange-500/10 to-orange-500/5 rounded-3xl"}`}
              >
                ...
                <div>
                  ...
                  <p
                    className={`text-4xl font-black ${isCyber ? "font-mono text-[var(--primary)]" : "text-orange-500"}`}
                  >
                    {stats.streak}
                  </p>
                </div>
              ...
              <div
                className={`p-6 flex items-center gap-6 ${isCyber ? "border border-[var(--primary)] bg-[var(--primary)]/5" : "bg-gradient-to-br from-yellow-500/10 to-yellow-500/5 rounded-3xl"}`}
              >
                ...
                <div>
                  ...
                  <p
                    className={`text-4xl font-black ${isCyber ? "font-mono text-[var(--primary)]" : "text-yellow-500"}`}
                  >
                    {stats.xp}
                  </p>
                </div>
          ...
          <div
            className={`p-6 flex flex-col justify-center gap-4 ${isCyber ? "border border-[var(--primary)] bg-[var(--primary)]/5" : "bg-gradient-to-br from-blue-500/10 to-blue-500/5 rounded-3xl"}`}
          >
            <div className="flex justify-between items-center w-full">
              ...
              <span
                className={`px-2 py-0.5 text-[10px] font-black uppercase rounded ${
                  stats?.role === "premium" || stats?.role === "admin"
                ...
                <div
                  ...
                  style={{
                    width:
                      stats?.role === "premium" || stats?.role === "admin"
                        ? "100%"
                        : `${((stats?.usage?.generations || 0) / 5) * 100}%`,
                  }}
                />
              </div>
            </div>
        ...
        {exams.length === 0 ? (
          <div className="card-premium text-center py-24">
            <div
              className={`w-20 h-20 mx-auto flex items-center justify-center mb-8 ${isCyber ? "border border-[var(--primary)] text-[var(--primary)]" : "bg-[var(--primary)]/5 text-[var(--primary)] rounded-full"}`}
            >
            ...
            <p className="text-[var(--text-muted)] mb-10 max-w-md mx-auto">
              Prześlij swój pierwszy dokument PDF, aby wygenerować plan nauki.
            </p>
            ...
            {exams.map((exam) => (
              ...
              >
                <div className="flex justify-between items-start mb-8 z-10">
                  <div
                    className={`p-4 ${isCyber ? "border border-[var(--primary)] text-[var(--primary)]" : "bg-[var(--primary)]/10 text-[var(--primary)] rounded-2xl"}`}
                  >
                ...
                {renamingId === exam._id ? (
                  ...
                  <Link href={`/exam/${exam._id}`} className="block">
                    <h3
                      className={`text-2xl font-bold mb-4 group-hover:text-[var(--primary)] transition-colors line-clamp-2 leading-tight ${isCyber ? "font-mono" : "font-sans"}`}
                    >
                ...
                <Link
                  href={`/exam/${exam._id}`}
                  className="flex items-center justify-between mt-auto pt-6 border-t border-[var(--border)] z-0"
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider py-1 px-3 ${
                        exam.status === "ready"
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${exam.status === "ready" ? "bg-green-500" : "bg-amber-500 animate-pulse"}`}
                      />
