...
export default function CreateExam() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async (acceptedFiles) => {
      console.log(
        `[Create] Wybrano ${acceptedFiles.length} nowych plików. Odczytywanie do pamięci...`,
      );
      try {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const item = files[i];
        console.log(
          `[Create] Przesyłanie pliku ${i + 1}/${files.length}: ${item.name} (${item.type})`,
        );
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-6 py-20 w-full flex-1">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-extrabold mb-4 tracking-tight text-[var(--foreground)]">
            {getLabel("newPlan")}
          </h1>
          <p className="text-[var(--text-muted)] font-medium text-lg leading-relaxed">
            {isCyber
              ? "// Prześlij pliki PDF z materiałami, a my zajmiemy się resztą."
              : "Prześlij swoje materiały, aby wygenerować spersonalizowaną ścieżkę nauki."}
          </p>
        </div>
        ...
        <div className={`card-premium space-y-10 ${isCyber ? "" : "rounded-[2rem] shadow-xl"}`}>
          ...
          <div
            className={`p-6 border-2 flex flex-col gap-6 transition-all ${isSpeedrun ? (isCyber ? "border-amber-500 bg-amber-500/5 shadow-[0_0_20px_rgba(245,158,11,0.2)]" : "border-amber-500 bg-amber-50/50 rounded-3xl shadow-lg shadow-amber-500/10") : "border-[var(--border)] hover:border-[var(--primary)]/30"} ${isCyber ? "" : "rounded-3xl"}`}
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 ${isSpeedrun ? "bg-amber-500 text-black" : "bg-[var(--surface)] text-[var(--text-muted)]"} ${isCyber ? "" : "rounded-xl"}`}
                >
          ...
          <div
            {...getRootProps()}
            className={`border-2 border-dashed p-12 text-center transition-all cursor-pointer group ${
              isDragActive
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <div
                className={`p-5 mb-6 transition-transform group-hover:scale-110 ${isCyber ? "border border-[var(--primary)] text-[var(--primary)]" : "bg-[var(--primary)]/10 text-[var(--primary)] rounded-2xl"}`}
              >
              ...
              <p className="text-sm text-[var(--text-muted)] font-medium">
                {isCyber
                  ? "// Obsługujemy pliki PDF (możesz dodać wiele)"
                  : "Kliknij lub przeciągnij tutaj pliki PDF z materiałami."}
              </p>
            </div>
          </div>
          ...
          {error && (
            <div
              className={`flex items-center gap-3 p-5 border border-red-500/50 bg-red-500/5 text-red-600 font-bold text-sm ${isCyber ? "" : "rounded-2xl"}`}
            >
              <AlertCircle className="w-6 h-6" />
              {error}
            </div>
